...
ClickUp API Client
==================
Wrapper thread-safe per le chiamate all'API ClickUp v2.
...
- Retry automatico su 429/5xx con backoff esponenziale
- Timeout configurabile
- Logging coerente con il resto del progetto
"""
...
class ClickUpClient:
    ...
    Istanziarlo con `ClickUpClient.from_app_config(app)` oppure direttamente
    con token + list_id.
    """
    ...
    def __init__(
        ...
        if not list_id:
            raise ValueError("CLICKUP_LIST_ID mancante")
    ...
    def from_app_config(cls, app=None) -> "ClickUpClient":
        ...
        return cls(
            ...
            list_id=app.config.get("CLICKUP_LIST_ID", ""),
        ...
        while attempt <= self.max_retries:
            ...
            try:
                if files is not None:
                    # multipart upload: non forzare Content-Type JSON
                ...
                last_exc = exc
                logger.warning(
                    "[ClickUp] %s %s fallita (tentativo %d/%d): %s",
                    method,
                    path,
                    attempt,
                    self.max_retries + 1,
            ...
            if resp.status_code == 429:
                retry_after = float(resp.headers.get("Retry-After", self._backoff(attempt)))
                logger.warning("[ClickUp] rate limited, retry after %.1fs", retry_after)
                if attempt > self.max_retries:
                    raise ClickUpError("Rate limited", status_code=429, response_text=resp.text)
                time.sleep(retry_after)
                continue

            if 500 <= resp.status_code < 600:
                logger.warning(
                    "[ClickUp] %s %s → %d (tentativo %d/%d)",
                    method,
                    path,
                    resp.status_code,
                    attempt,
