...
def generate_report():
    ...
    with app.app_context():
        ...
        # Join con WeeklyCheck per ottenere cliente_id
        ...
        for row in query:
            (resp_id, weekly_check_id, submit_date,
             ...
             progress_rating,
             cliente_id, nutrizionista_id, coach_id, psicologa_id,
             data_inizio_nutrizione, data_inizio_coach, data_inizio_psicologia,
             data_scadenza_nutrizione, data_scadenza_coach, data_scadenza_psicologia) = row
            
            # Determina per ogni professionista se è presente nella risposta
            # Nutrizionista
            if nutritionist_user_id is not None:
                prof_id = nutritionist_user_id
                rating = nutritionist_rating
                start_date = data_inizio_nutrizione
                end_date = data_scadenza_nutrizione
                dept = TeamTypeEnum.nutrizione
            # Coach
            elif coach_user_id is not None:
                prof_id = coach_user_id
                rating = coach_rating
                start_date = data_inizio_coach
                end_date = data_scadenza_coach
                dept = TeamTypeEnum.coach
            # Psicologo
            elif psychologist_user_id is not None:
                prof_id = psychologist_user_id
                rating = psychologist_rating
                start_date = data_inizio_psicologia
                end_date = data_scadenza_psicologia
                dept = TeamTypeEnum.psicologia
            else:
                continue  # Nessun professionista associato
            ...
            # Calcola check aspettati per questo cliente
            if start_date is not None:
                expected = compute_expected_checks(start_date, end_date)
                stats['expected_checks'] += expected
            ...
            if team is not None:
                ...
                if start_date is not None:
                    expected = compute_expected_checks(start_date, end_date)
                    team_stats['expected_checks'] += expected
            ...
            if start_date is not None:
                expected = compute_expected_checks(start_date, end_date)
                dept_stats['expected_checks'] += expected
        ...
        output_file = 'report_statistiche_check.xlsx'
