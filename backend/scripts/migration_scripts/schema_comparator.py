...
def process_alter_stmt(stmt, constraint_list, outfile):
    """
    Decides whether an ALTER TABLE statement is a constraint that should be delayed
    or a structural change (like setting defaults) that should be written immediately.
    """
    lower_stmt = stmt.lower()
    if 'add constraint' in lower_stmt or 'primary key' in lower_stmt or 'foreign key' in lower_stmt:
        constraint_list.append(stmt)
    else:
        outfile.write(stmt)
...
def generate_migrated_dump(new_schema_path, old_dump_path, output_path, new_schema_def):
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as outfile:
            ...
            # 1. Copy Structure from New Suite (Filtering out COPY/INSERT/CONSTRAINTS)
            print(f"Reading Schema from {new_schema_path}...")
            outfile.write(f"-- BASE SCHEMA DEFINITION\n")
            ...
            with open(new_schema_path, 'r', encoding='utf-8') as infile:
                ...
                for line in infile:
                    ...
                    # Multi-line ALTER TABLE handling
                    if line.startswith('ALTER TABLE ONLY '):
                        in_alter_block = True
                        current_alter_stmt = [line]
                        if line.strip().endswith(';'):
                            in_alter_block = False
                            process_alter_stmt(line, constraint_lines, outfile)
                        continue
                    
                    if in_alter_block:
                        current_alter_stmt.append(line)
                        if line.strip().endswith(';'):
                            in_alter_block = False
                            full_stmt = "".join(current_alter_stmt)
                            process_alter_stmt(full_stmt, constraint_lines, outfile)
                        continue

                    # CREATE INDEX handling (usually single line but can be multi)
                    if line.startswith('CREATE INDEX ') or line.startswith('CREATE UNIQUE INDEX '):
                        if line.strip().endswith(';'):
                            constraint_lines.append(line)
                        else:
                            in_alter_block = True # Reuse flag for index too
                            current_alter_stmt = [line]
                        continue
            ...
            # NOTE: session_replication_role is removed as it's not allowed on many managed DBs
                ...
                for line in process.stdout:
                    ...
                    if current_table and current_cols:
                        ...
                        if target_cols:
                            ...
                            if current_batch_table != current_table or current_batch_cols != cols_tuple:
                                # Flush existing batch if table or columns changed
                                if current_batch_vals:
                                    cols_str = ", ".join(current_batch_cols)
                                    vals_str = ", ".join(current_batch_vals)
                                    outfile.write(f"INSERT INTO public.{current_batch_table} ({cols_str}) VALUES {vals_str};\n")
                                    current_batch_vals = []
                                
                                current_batch_table = current_table
                                current_batch_cols = cols_tuple

                            # Add value group to batch
                            vals_group = f"({', '.join(target_vals)})"
                            current_batch_vals.append(vals_group)
                            
                            # Flush if batch size reached
                            if len(current_batch_vals) >= 1000:
                                cols_str = ", ".join(current_batch_cols)
                                vals_str = ", ".join(current_batch_vals)
                                outfile.write(f"INSERT INTO public.{current_batch_table} ({cols_str}) VALUES {vals_str};\n")
                                current_batch_vals = []

            # Flush any remaining batch at the end of the loop
            if current_batch_vals:
                cols_str = ", ".join(current_batch_cols)
                vals_str = ", ".join(current_batch_vals)
                outfile.write(f"INSERT INTO public.{current_batch_table} ({cols_str}) VALUES {vals_str};\n")

            # 3. Finalize: Write Constraints and Indexes
            print("\nFinalizing Schema (Adding Constraints and Indexes)...")
            outfile.write(f"\n\n-- SCHEMA CONSTRAINTS AND INDEXES (Delayed)\n")
...
if __name__ == "__main__":
    ...
    # Overrides from CLI
    if len(sys.argv) > 1:
        OLD_SUITE_BACKUP = sys.argv[1]
    if len(sys.argv) > 2:
        NEW_SUITE_BACKUP = sys.argv[2]
    if len(sys.argv) > 3:
