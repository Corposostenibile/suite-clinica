...
Eseguire una volta sola:
    flask shell < scripts/migration_scripts/migrate_old_tickets.py
oppure nel pod:
...
def migrate():
    ...
    # ── Trova ticket aperti (non chiusi) ──
    open_statuses = [
        TicketStatusEnum.nuovo,
        TicketStatusEnum.in_lavorazione,
    ...
    print(f"  Ticket totali nel vecchio sistema: {total_count}")
    print()
    ...
    for old in old_tickets:
        try:
            # Verifica se già migrato (cerca per vecchio ticket_number nel titolo o description)
            ...
            if new_status == TeamTicketStatusEnum.chiuso and old.closed_at:
                new_ticket.closed_at = old.closed_at

            db.session.add(new_ticket)
            db.session.flush()  # per avere new_ticket.id
            ...
            # ── Messaggi (da TicketComment + TicketMessage) ──
            # Prima i commenti
            comments = (
                TicketComment.query
                .filter_by(ticket_id=old.id)
            ...
            # ── Status changes ──
            old_changes = (
                OldStatusChange.query
                .filter_by(ticket_id=old.id)
                .order_by(OldStatusChange.created_at.asc())
                .all()
            )
            for sc in old_changes:
                ...
                new_sc = TeamTicketStatusChange(
                    ...
                    from_status=from_s,
                    to_status=to_s,
                    changed_by_id=sc.changed_by_id,
                )
                new_sc.created_at = sc.created_at
                db.session.add(new_sc)

            migrated += 1
            print(f"  OK  {old.ticket_number} → {new_ticket.ticket_number}  "
                  f"[{old.status.value} → {new_status.value}]  "
                  f"{len(assignee_ids)} assegnatari, "
                  f"{len(comments)+len(messages)} messaggi, "
                  f"{len(old_attachments) + (1 if old.attachment_filename else 0)} allegati")
