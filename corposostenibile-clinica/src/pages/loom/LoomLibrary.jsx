...
const LoomLibrary = () => {
  ...
  const stats = useMemo(() => {
    const associated = recordings.filter((recording) => Boolean(recording?.cliente_id)).length;
    return {
  ...
  return (
    <div className="container-fluid p-0">
      ...
      <div className="loomlib-view-tabs">
        <button className={`loomlib-view-tab${associationFilter === 'all' ? ' active' : ''}`} onClick={() => setAssociationFilter('all')}>
          <i className="ri-folder-video-line"></i>
          Tutti
          <span className="loomlib-view-count">{stats.total}</span>
        </button>
        <button className={`loomlib-view-tab${associationFilter === 'associated' ? ' active' : ''}`} onClick={() => setAssociationFilter('associated')}>
          <i className="ri-user-heart-line"></i>
          Associati a paziente
          <span className="loomlib-view-count">{stats.associated}</span>
        </button>
        <button className={`loomlib-view-tab${associationFilter === 'unassociated' ? ' active' : ''}`} onClick={() => setAssociationFilter('unassociated')}>
      ...
      <div className="loomlib-filter-bar">
        ...
        <div className="loomlib-filter-controls">
          {canUseCapienzaStyleFilters && (
            <select className="loomlib-filter-select" value={teamFilter} onChange={(e) => setTeamFilter(e.target.value)}>
          ...
          />
          <select className="loomlib-filter-select" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)}>
                    ...
                    <tbody>
                      {group.rows.map((recording) => (
                        <tr key={recording.id}>
                          ...
                          {isAdminOrTeamLeader && <td>{recording.submitter_user_name || '-'}</td>}
                          <td>{formatDateTime(recording.created_at)}</td>
                          <td style={{ textAlign: 'right' }}>
                            <div className="loomlib-actions">
                              <a href={recording.loom_link} target="_blank" rel="noopener noreferrer" className="loomlib-open-link">
                                <i className="ri-external-link-line"></i> Apri
                              </a>
                              <button type="button" className="loomlib-copy-btn" onClick={() => handleCopyLink(recording.loom_link)}>
                                <i className="ri-file-copy-line"></i> Copia
                              </button>
                              <button type="button" className="loomlib-copy-btn" onClick={() => setPreviewLink(recording.loom_link)}>
      ...
      {sortedRecordings.length > 0 && (
        <div className="loomlib-pagination">
          <button type="button" className="loomlib-page-btn" onClick={() => setPage((prev) => Math.max(1, prev - 1))} disabled={page <= 1}>
            <i className="ri-arrow-left-s-line"></i> Precedente
          </button>
          <span>Pagina {page} / {totalPages}</span>
          <button type="button" className="loomlib-page-btn" onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))} disabled={page >= totalPages}>
