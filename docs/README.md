# Documentazione — Suite Clinica Corposostenibile

Questa cartella contiene tutta la documentazione del progetto **Suite Clinica Corposostenibile**.

La documentazione è organizzata per aree tematiche ed è pensata per essere utile sia al **team di sviluppo** che ai **professionisti coinvolti nell'operatività**.

---

## Indice

### 📋 Panoramica Generale
- [Panoramica del Progetto](./00-panoramica/overview.md) — Visione generale, architettura, ruoli, flussi principali

### 🏗️ Infrastruttura & Deploy
- [Analisi CI/CD](./01-infrastruttura/ci_cd_analysis.md)
- [Setup Infrastruttura GCP](./01-infrastruttura/gcp_infrastructure_setup_report.md)
- [Compliance Infrastruttura](./01-infrastruttura/infrastructure_compliance_report.md)
- [Rapporto Infrastruttura 2026](./01-infrastruttura/rapporto_infrastruttura_2026.md)
- [Procedura Migrazione](./01-infrastruttura/procedura_migrazione.md)
- [DuckDNS Local Dev VPS](./01-infrastruttura/duckdns_local_dev_vps.md)

### 🏥 Clienti Core
- [Piano Medico Nutrizione](./03-clienti-core/piano_medico_nutrizione.md)
- [Test Medico Passo-Passo](./03-clienti-core/test_medico_passo_passo.md)

### ⚙️ Sviluppo e Varie
- [Refactor Plan](./06-sviluppo-e-varie/refactor_status_report.md)
- [Refactoring Notes](./06-sviluppo-e-varie/refining_refactor_plan.md)

---

## Come Contribuire alla Documentazione

1. Ogni documento segue il template standard in `00-panoramica/template-documento.md` (se disponibile)
2. **Lingua**: tutta in italiano
3. **Formato**: Markdown puro (`.md`)
4. La documentazione vive nel branch `docs/project-documentation` e viene integrata su `main` a milestone completate

---

*Ultimo aggiornamento: Marzo 2026*
