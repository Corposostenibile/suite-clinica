...
update_env_database_url() {
    ...
    local frontend_env_file="$PROJECT_DIR/corposostenibile-clinica/.env.local"
    ...
    # Frontend (Vite) proxy target backend, per dev locale multi-utente
    local new_vite_backend_url="http://127.0.0.1:$port"
    if [[ -f "$frontend_env_file" ]]; then
        if grep -q "^VITE_BACKEND_URL=" "$frontend_env_file"; then
            sed -i "s|^VITE_BACKEND_URL=.*|VITE_BACKEND_URL=$new_vite_backend_url|" "$frontend_env_file"
        else
            echo "VITE_BACKEND_URL=$new_vite_backend_url" >> "$frontend_env_file"
        fi
    else
        cat > "$frontend_env_file" <<EOF
# Auto-generated by dev.sh for local development
VITE_BACKEND_URL=$new_vite_backend_url
EOF
    fi
    log_success "VITE_BACKEND_URL aggiornato per $dev: $new_vite_backend_url"
}
